...
const Option: React.FC = () => {
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      try {
        const response = await apiClient.get(`/order/option/?mealId=${state?.mealId}&tabIndex=${state?.tabIndex}&day=${state?.menuIndex}`);
  ...
  useEffect(() => {
    ...
    const selectedBasePrice = baseOptions.find((opt) => opt.id === selectedBase)?.difference || 0;
    const selectedProteinPrice = proteinOptions.find((opt) => opt.id === selectedProtein)?.difference || 0;
    const selectedVegPrice = vegOptions.find(opt => opt.id === selectedVeg)?.difference || 0;
    const selectedFlavorPrice = flavorOptions.find((opt) => opt.id === selectedFlavor)?.difference || 0;

    console.log(selectedBasePrice, selectedProteinPrice, selectedVegPrice, selectedFlavorPrice);

    const additionalProteinPrice =
      additionalProtein.reduce(
        (acc, id) =>
          acc +
          (proteinOptions
            .find((opt) => opt.id === id)
    ...
    const additionalVegPrice =
      additionalVeg.reduce(
        (acc, id) =>
          acc +
          (vegOptions
            .find((opt) => opt.id === id)
    ...
    const additionalFlavorPrice =
      additionalFlavor.reduce(
        (acc, id) =>
          acc +
          (flavorOptions
            .find((opt) => opt.id === id)
